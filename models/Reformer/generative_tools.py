...
class TrainingWrapperTM(nn.Module):
    def __init__(self, net, ignore_index = -100, pad_value = 0):
        super().__init__()
        assert isinstance(net, ReformerTM), 'generative trainer wrapper can only accept ReformerTM class'
    ...
    def forward(self, x, return_loss = False, **kwargs):
        ...
        if not return_loss:
            if not isinstance(x, torch.Tensor):
                x = pad(x)
            return self.net(x, **kwargs) ## 이거 리턴됨
