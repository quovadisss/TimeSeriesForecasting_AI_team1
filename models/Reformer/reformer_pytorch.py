...
from models.Reformer.reversible import ReversibleSequence
...
class ReformerTM(nn.Module):
    def __init__(self, dim, depth, seq_len, heads = 8, dim_head = 64, bucket_size = 25, n_hashes = 4, ff_chunks = 100, attn_chunks = 1, causal = False, weight_tie = False, lsh_dropout = 0., ff_dropout = 0., ff_mult = 4, ff_activation = None, ff_glu = False, post_attn_dropout = 0., layer_dropout = 0., random_rotations_per_head = False, use_scale_norm = False, use_rezero = False, use_full_attn = False, full_attn_thres = 0, reverse_thres = 0, num_mem_kv = 0, one_value_head = False, emb_dim = None, return_embeddings = False, weight_tie_embedding = False, fixed_position_emb = False, absolute_position_emb = False, rotary_emb = True, axial_position_shape = None, n_local_attn_heads = 0, pkm_layers = tuple(), pkm_num_keys = 128):
        super().__init__()
        ...
        if rotary_emb:
            ...
            self.pos_emb = AbsolutePositionalEmbedding(emb_dim, seq_len)
        elif fixed_position_emb:
            self.pos_emb = FixedPositionalEmbedding(emb_dim)
        else:
            axial_position_shape = default(axial_position_shape, (math.ceil(seq_len / bucket_size), bucket_size))
    ...
    @torch.no_grad()
    def generate(self, label_seq, pred_len,  **kwargs):
        was_training = self.reformer.training
        input_dim = label_seq.shape[-1]
        label_len = label_seq.shape[1]
        
        self.reformer.eval()

        # out = [batch_size , label + pred len , seq dim]
        out = torch.zeros([label_seq.shape[0], label_len + pred_len , input_dim] )
        out = torch.cat([label_seq, out], dim=1).float()
        ...
        for i in range(pred_len):
            x = out[:, -self.seq_len:]
            input_mask = input_mask[:, -self.seq_len:]

            logits = self.reformer(x, input_mask=input_mask, **kwargs)[:, -1, :]

            out[:,i+label_len,:] = torch.cat((out, logits), dim=-1)
        ...
        self.reformer.train(was_training)
        return out
